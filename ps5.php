...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'add_to_cart') {
        ...
        $checkStmt = $conn->prepare("SELECT quantity FROM ps5_games WHERE game_name = ?");
        $checkStmt->bind_param("s", $game_name);
        $checkStmt->execute();
        ...
        // Decrement the quantity in the ps5_games table
        $updateStmt = $conn->prepare("UPDATE ps5_games SET quantity = quantity - 1 WHERE game_name = ? AND quantity > 0");
        $updateStmt->bind_param("s", $game_name);
        $updateStmt->execute();
    ...
    if ($action == 'remove_from_cart') {
        ...
        $getStmt = $conn->prepare("SELECT game_name, quantity FROM cart_items WHERE id = ? AND user_id = ?");
        $getStmt->bind_param("ii", $cart_id, $user_id);
        $getStmt->execute();
        $result = $getStmt->get_result();
        
        if ($item = $result->fetch_assoc()) {
            // Increment the quantity back to the ps5_games table
            $updateStmt = $conn->prepare("UPDATE ps5_games SET quantity = quantity + ? WHERE game_name = ?");
            $updateStmt->bind_param("is", $item['quantity'], $item['game_name']);
            $updateStmt->execute();
    ...
    if ($action == 'search') {
        ...
        $stmt = $conn->prepare("SELECT game_name, price, emoji FROM ps5_games WHERE game_name LIKE ?");
...
// Get all games from database to display
$games = [];
$game_result = $conn->query("SELECT * FROM ps5_games WHERE game_name != 'PlayStation 5'");
...
<head>
    ...
    <style>
        ...
        :root{
            ...
            --dark-bg:#0d0d0d;
            --light-text:#ffffff;
            --glow-shadow: 0 0 10px var(--neon-blue) , 0 0 20px var(--neon-blue) , 0px 0px 40px var(--neon-blue);
        }
        ...
        *{
        ...
        /* Features Section (Unused but preserved) */
        .features{
            padding: 6rem 5%;
            background-color: rgba(13, 13, 13, 0.9);
        }
...
<body>
    ...
    <section class="platform" id="home">
        <div class="platform-content">
            ...
            <p>Join the ultimate community and dominate the leaderboard. Whether you're a casual player or a pro, we have got something for you</p>
            <h2 id="h2">just for $999.99</h2>
            <br>
            <button class="buy-btn" onclick="addToCart('PlayStation 5', 999.99)">Add to Cart</button>
        </div>
        <div class="platform-img">
            <img src="https://gmedia.playstation.com/is/image/SIEPDC/ps5-product-thumbnail-01-en-14sep21?$facebook$" alt="PlayStation 5 Console" style="width: 100%; height: 100%; object-fit: cover; border-radius: 20px;">
        </div>
    </section>
                ...
                <div class="games-grid" id="gamesGrid">
                    ...
                    <div class="game-card">
                        ...
                        <div class="game-card-content">
                            ...
                            <button class="game-card-btn" onclick="addToCart('<?php echo $game['game_name']; ?>', <?php echo $game['price']; ?>)">
                                Add to Cart
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Function to Close Cart Dropdown
        function closeCart() {
            const cartDropdown = document.getElementById('cartDropdown');
        ...
        const totalCards = <?php echo count($games); ?>;
